...
uint8_t VailAdapter::sendMidiEventDeltaTimeHigh(uint8_t header, uint8_t key) {
    ...
    // Send a CC (0xB0) event with the high 7 bits of the interval
    midiEventPacket_t event = {header, 0xB0, key, delta_high};
...
if (this->sendMidiEventDeltaTimes)
    velocity = sendMidiEventDeltaTimeHigh(header, key);
